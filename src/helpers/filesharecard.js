...
function render(fileInfo, shareInfo, share_id) {
    ...
    name = name.replace(/</g, "&lt;"); // escape HTML tag brackets
    name = name.replace(/>/g, "&gt;"); // escape HTML tag brackets

    let type = fileInfo.attributes.type.split('/')[0];
    console.log(type);

    if (shareInfo.attributes.shared_type === 0 && type === 'image' && shareInfo.attributes.downloadUrl) {

        ret += '<img class="xbbcode-img" alt="' + name + '" src="' + shareInfo.attributes.downloadUrl + '" />'
        ...
        if (shareInfo.attributes.shared_type === 2 && shareInfo.attributes.paid === false) {
            ret += '<div class="xbbcode-flieshare-block-protected">' + '付费分享' + '</div>'; 
        } else if (shareInfo.attributes.shared_type === 2 && shareInfo.attributes.paid === true) {
            ret += '<div class="xbbcode-flieshare-block-protected">' + '[可下载] 付费分享' + '</div>'; 
